...
function format_amount($amount, $currency)
{
    ...
    if (strtoupper($currency) === 'XRP') {
        $decimals = 2;
    } elseif (strtoupper($currency) === 'TRX' || strtoupper($currency) === 'ADA') {
        $decimals = 6;
    } elseif (strtoupper($currency) === 'XLM') {
        $decimals = 7;
    }
...
{
    // Используем современную библиотеку chillerlan/php-qrcode
    ...
    // Генерируем QR-код напрямую как base64
    $base64 = $qrcode->render($data);

    // Удаляем префикс данных base64, который добавляет библиотека
...
function isMemo($currency)
{
    $memo_currencies = ['XRP', 'XLM', 'XEM', 'XMR', 'DASH', 'ZEC', 'TON']; // Монеты, требующие мемо
    return in_array($currency, $memo_currencies);
}
